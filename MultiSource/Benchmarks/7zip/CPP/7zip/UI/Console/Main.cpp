...
static void ShowMessageAndThrowException(CStdOutStream &s, LPCSTR message,
                                         NExitCode::EEnum code) {
  s << message << endl;
  throw code;
}
...
#ifndef _WIN32
static void GetArguments(int numArgs, const char *args[],
                         UStringVector &parts) {
  parts.Clear();
...
int Main2(
  ...
  extern void mySplitCommandLine(int numArgs, const char *args[],
                                 UStringVector &parts);
  ...
  if (options.Command.CommandType == NCommandType::kInfo) {
    ...
    if (options.Method.CompareNoCase(L"CRC") == 0) {
      HRESULT res = CrcBenchCon((FILE *)stdStream, options.NumIterations,
...
#endif
          (FILE *)stdStream, options.NumIterations, options.NumThreads,
    ...
    if (isExtractGroupCommand) {
      ...
      HRESULT result =
          DecompressArchives(codecs, formatIndices, options.ArchivePathsSorted,
                             options.ArchivePathsFullSorted,
                             options.WildcardCensor.Pairs.Front().Head, eo,
                             &openCallback, ecs, errorMessage, stat);
      ...
      UInt64 numErrors = 0;
      HRESULT result = ListArchives(codecs, formatIndices, options.StdInMode,
                                    options.ArchivePathsSorted,
                                    options.ArchivePathsFullSorted,
                                    options.WildcardCensor.Pairs.Front().Head,
                                    options.EnableHeaders, options.TechMode,
#ifndef _NO_CRYPTO
    ...
    if (!uo.Init(codecs, formatIndices, options.ArchiveName))
      throw kUnsupportedArcTypeMessage;
    HRESULT result = UpdateArchive(codecs, options.WildcardCensor, uo,
    ...
    if (callback.CantFindFiles.Size() > 0) {
      ...
      for (int i = 0; i < numErrors; i++) {
        ...
        stdStream << NError::MyFormatMessageW(callback.CantFindCodes[i])
                  << endl;
      }
      stdStream << "----------------" << endl;
