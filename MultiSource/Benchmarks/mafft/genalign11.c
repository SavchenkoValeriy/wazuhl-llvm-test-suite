...
#if 1
static void match_calc_gena( float *match, char **s1, char **s2, int i1, int lgth2 ) 
{
	char tmpc = s1[0][i1];
	char *seq2 = s2[0];
...
#if 0
static void match_calc_gena_bk( float *match, float **cpmx1, float **cpmx2, int i1, int lgth2, float **floatwork, int **intwork, int initialize )
{
	int j, k, l;
	float scarr[26];
...
//	fprintf( stderr, "@@@@@@@@@@@@@ penalty_OP = %f, penalty_EX = %f, pelanty = %f\n", fpenalty_OP, fpenalty_EX, fpenalty );
	...
	match_calc_gena( initverticalw, seq2, seq1, 0, lgth1 );

	match_calc_gena( currentw, seq1, seq2, 0, lgth2 );
...
fprintf( stderr, "\n" );
	...
	for( i=1; i<lasti; i++ )
	{
		...
		match_calc_gena( currentw, seq1, seq2, i, lgth2 );
#if DEBUG2
